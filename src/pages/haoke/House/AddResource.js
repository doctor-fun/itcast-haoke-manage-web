...
@Form.create()  //生成form，可以使用它的api，
  //handleSubmit拦截了提交请求，对拦截的结果进行了验证
        ...
        form.validateFieldsAndScroll((err, values) => {
            if (!err) {
              ...
              if(values.floor_1 && values.floor_2){
                values.floor = values.floor_1 + "/" + values.floor_2;//使得格式变成2/8什么的，最后删除它
...
//onSubmit拦截了submit请求，提交到服务端前对数据进行处理
                ...
                <Form onSubmit={this.handleSubmit} hideRequiredMark style={{ marginTop: 8 }}>
                    <Card bordered={false} title="房源信息">
                        <FormItem {...formItemLayout} label="楼盘名称">
                            <AutoComplete
                              style={{ width: '100%' }}
                              dataSource={this.state.estateDataSource}//刚开始是个空的数组
                              placeholder="搜索楼盘"
                              onSelect={(value, option)=>{
                                let v = estateMap.get(value);//从estateMap当中拿到值
                                this.setState({
                                  estateAddress: v.substring(v.indexOf('|')+1),//楼盘id要被去除掉
                                  estateId : v.substring(0,v.indexOf('|'))
                                });
                              }}
                              onSearch={this.handleSearch}//调用onsearch方法
                        ...
                        <FormItem {...formItemLayout} label="支付方式">
                          ...
                          //required true表示此项为必须项，如果不填则会给出提示
                            //initialValue表示默认值
                            //getFieldDecorator对变量进行双向数据绑定
                          (
